(function () {
    ...
    inputCapa.addEventListener('change', () => {
        ...
        // Valida imediatamente ao selecionar — bloqueia preview se inválido
        if (!validarELimparInput(file, inputCapa)) {
            capaPreview.style.display = 'none';
            capaPlaceh.style.display  = '';
            return;
        }
    ...
    document.querySelectorAll('.foto-slot').forEach(slot => {
        ...
        input.addEventListener('change', () => {
            ...
            // Valida imediatamente — bloqueia preview se inválido
            if (!validarELimparInput(file, input)) return;

            // Remove span de "+" e exibe preview
    ...
    document.getElementById('form-criar').addEventListener('submit', async e => {
        ...
        // Validação defensiva da capa (caso o change event tenha sido burlado)
        const fileCapa = inputCapa.files[0];
        if (fileCapa) {
            const erro = validarArquivoImagem(fileCapa);
            if (erro) { toast(erro, 'error'); return; }
        }

        // Validação defensiva das fotos extras
        const inputsFotos = document.querySelectorAll('.input-foto-extra');
        for (const inp of inputsFotos) {
